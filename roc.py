...
if __name__=="__main__":
	...
	parser.add_argument('--train_input', nargs='+',required=True)
	parser.add_argument('--label_dict')
	parser.add_argument('--label_index', nargs='+', type=int)
	parser.add_argument('--x_end_index', nargs='+', type=int)
	...
	#if type(x_end_index)==list:
	all_fprs=[]
	mean_tprs=[]
	roc_aucs=[]
	for model, x_end, label, train in zip(args.models, x_end_index, label_index, train_inp):
	...
	plt.xlim((0,1.05))
	plt.ylim((0,1.05))
	colors = cycle(['blue', 'darkorange', 'aqua'])
	for i,color in zip(range(len(args.model_names)),colors):
		plt.plot(all_fprs[i],mean_tprs[i],color=color,
			label='{0}'
             		''.format(args.model_names[i]))
	plt.plot([0, 1], [0, 1], 'k--')
	plt.xlabel('False Positive Rate')
	plt.ylabel('True Positive Rate')
	plt.title('ROC Curve of Class Lactation Stage')
